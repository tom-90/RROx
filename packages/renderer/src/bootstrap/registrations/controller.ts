...
export class RegistrationController<E extends RegistrationType<( ...params: unknown[] ) => void>> extends EventEmitter implements IRegistrationController<E> {
    ...
    unregisterPlugin( pluginName: string ) {
        const toUnregister = this.registrations.filter( ( reg ) => reg.metadata.plugin === pluginName );

        if( toUnregister.length > 0 ) {
            this.registrations = this.registrations.filter( ( r ) => !toUnregister.includes( r ) );

            for( let reg of toUnregister )
                this.emit( 'unregister', reg );
        }
    }
