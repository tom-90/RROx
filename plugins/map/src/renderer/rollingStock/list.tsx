            ...
            <PageContent style={{ maxWidth: 1200 }}>
                <Tabs defaultActiveKey="1" style={{ margin: '0 10px' }} centered>
                    ...
                    <Tabs.TabPane tab="Cabooses &#38; Miscellaneous" key="4" style={{ height: 'calc(100vh - 200px)', overflow: 'auto' }}>
                        <FramesList
                            data={world?.frameCars.map( ( frame, index ) => ( { frame, index } ) ).filter( ( { frame } ) => frame.type === FrameCarType.CABOOSE || frame.type === FrameCarType.WAYCAR || frame.type === FrameCarType.PLOW || frame.type === FrameCarType.COACH_DSPRR_1 || frame.type === FrameCarType.COACH_DSPRR_2 ) ?? []}
                            onOpenControls={openControl}
                            onLocate={locate}
                        />
                    </Tabs.TabPane>
